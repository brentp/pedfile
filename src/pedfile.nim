...
proc LCA(a:Sample, b:Sample, lcas: var HashSet[sample_path], a_path:seq[Sample], b_path:seq[Sample], level:int=0) =
  ...
  if level > 10:
    raise newException(PedigreeError, &"[slivar/pedfile] likely pedigree cycle {a_path}, {b_path}")
  ...
  block:
    ...
    LCA(a.mom, b, lcas, a_path, b_path, level+1)
  block:
    var a_path = a_path & a.dad
    LCA(a.dad, b, lcas, a_path, b_path, level+1)
  block:
    var b_path = b_path & b.mom
    LCA(a, b.mom, lcas, a_path, b_path, level+1)
  block:
    var b_path = b_path & b.dad
    LCA(a, b.dad, lcas, a_path, b_path, level+1)
...
type Sampler* = concept v
  ...
  v.set_samples(seq[string])

proc match*(samples: seq[Sample], vcf:var Sampler, verbose:bool=true): seq[Sample] =
...
when isMainModule:
  ...
  var ped_samples = parse_ped("tests/testa.ped")

  type fakevcf = object
    s_samples: seq[string]

  proc set_samples(v:var fakevcf, samples:seq[string]) =
    v.s_samples = samples
  proc samples(v:var fakevcf): seq[string] =
    return v.s_samples

  var ovcf = fake_vcf(s_samples:"101976-101976	100920-100920	100231-100231	100232-100232	100919-100919	Totally_New_Sample".split())

  suite "pedfile test suite":

    test "that samples match those in vcf":
      var osamples = ped_samples.match(ovcf)
      for i, s in osamples:
        check s.i == i
        check s.id == ovcf.samples[i]

    test "that reversed samples match those in vcf":
      var osamples = ped_samples.reversed
    ...
    test "siblings":
      check ped_samples[0].id == "101976"
      for sib in ped_samples[0].siblings:
        check sib.dad == ped_samples[0].dad
        check sib.mom == ped_samples[0].mom
    ...
    test "relatedness with 603 ceph samples":
      var ceph_samples = parse_ped("tests/ceph.ped")
      var t = cpuTime()
      var n = 0

      for i, sampleA in ceph_samples[0..<ceph_samples.high]:
        for j, sampleB in ceph_samples[i + 1..ceph_samples.high]:
